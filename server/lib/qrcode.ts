 ...
 * @author davidshimjs, papnkukn, oli4jansen
...
const QRCodeLimitLength: [number, number, number, number][] = [
  [17, 14, 11, 7],
  [32, 26, 20, 14],
  [53, 42, 32, 24],
  [78, 62, 46, 34],
...
const QRUtil = {
  ...
  getLengthInBits: function (mode: QRMode, type: number) {
    if (1 <= type && type < 10) {
      switch (mode) {
...
class QRCodeModel {
  ...
  static createData(typeNumber: number, errorCorrectLevel: QRErrorCorrectLevel, dataList: any) {
...
export class QRCode {
  ...
  constructor(options?: string | Partial<QRCodeOptions>) {
    ...
    this.qrcode = new QRCodeModel(type, this.options.ecl);
  ...
  //Get type number
  _getTypeNumber(content: string, ecl: QRErrorCorrectLevel) {
